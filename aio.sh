...
install_x_i3() {
    echo "Installing Xorg and i3 window manager..."
    arch-chroot /mnt pacman -S --noconfirm xorg-server xorg-apps xorg-xinit xterm
    arch-chroot /mnt pacman -S --noconfirm i3 dmenu
    arch-chroot /mnt bash -c 'echo "exec i3" > /home/'"$sudo_user"'/.xinitrc'
    arch-chroot /mnt chown "$sudo_user:$sudo_user" /home/"$sudo_user"/.xinitrc
}

install_x_dwm() {
    echo "Installing Xorg and dwm window manager along with necessary dependencies..."
    arch-chroot /mnt pacman -S --noconfirm xorg-server xorg-apps xorg-xinit xterm
...
# Prompt for graphical environment installation choice
clear
echo "Choose an option for graphical interface installation:"
echo "1) Xorg with i3"
echo "2) Xorg with dwm"
echo "3) None"
read -p "Enter choice [1-3]: " GRAPHIC_OPTION
...
# Install graphical environment based on user's choice
case $GRAPHIC_OPTION in
    1)
        install_x_i3
        ;;
    2)
        ...
        ;;
    3)
        echo "Skipping graphical interface installation..."
        ;;
    *)
        echo "Invalid option, skipping graphical interface installation..."
        ;;
esac
...
echo "Your system is installed. It took $elapsed_minutes minutes and $elapsed_seconds seconds to install Arch Linux. Press enter to reboot."
read empty
# Reboot system
reboot
