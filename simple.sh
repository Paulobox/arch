...
# Configure the fastest mirrors based on your geographical location
echo "Configuring the fastest mirrors..."
reflector --verbose --latest 10 --protocol https --sort rate --save /etc/pacman.d/mirrorlist
if [ $? -ne 0 ]; then
    error "Failed to configure mirrors. Check your network connection or reflector settings."
fi
...
arch-chroot /mnt pacman -S --noconfirm git openssh networkmanager dhcpcd man-db man-pages pambase
...
sleep 2
arch-chroot /mnt pacman -S --noconfirm sudo bash-completion sshpass
...
echo "Your system is installed. Press enter to reboot "
read empty
exit
sleep 2
reboot
